...
class Lack {
	...
	public static function run()
	{
		...
		if (file_exists('./data/'.$template.'.'.config_item('lack_data_extension')))
		{
			// Get content
			self::$data['content'] = markdown(self::$ci->parser->parse(config_item('lack_data_path').'/'.$template.'.'.config_item('lack_data_extension'), self::$data, true));

			// And load view
			self::$ci->parser->parse(config_item('lack_template_path').'/layout', self::$data);
		}
		...
		elseif (is_dir('./data/'.$template))
		{
			...
			if (self::$ci->uri->segment(2) and file_exists('./data/'.self::$entry_dir.'/'.self::$ci->uri->segment(2).'.'.config_item('lack_data_extension')))
			{
				// Get content
				$entry = self::parse_entry(self::$ci->uri->segment(2).'.'.config_item('lack_data_extension'));
				self::$data['content'] = $entry['content'];
				
				// And load view
				if (file_exists(config_item('lack_templates').'/layout_'.$template.'.php'))
				{
					self::$ci->parser->parse(config_item('lack_template_path').'/layout_'.$template, self::$data);
				}
				else
				{
					self::$ci->parser->parse(config_item('lack_template_path').'/layout', self::$data);
			...
			else
			{
				...
				// And load view
				if (file_exists(config_item('lack_templates').'/layout_'.$template.'_list.php'))
				{
					self::$ci->parser->parse(config_item('lack_template_path').'/layout_'.$template.'_list', self::$data);
				}
				else
				{
					self::$ci->parser->parse(config_item('lack_template_path').'/layout_list', self::$data);
				}
			}
		}
	...
	public static function entry_list($format = 'html', $return = false)
	{
		if (self::$entry_files)
		{
			...
			{
				if (is_file(config_item('lack_data').'/'.self::$entry_dir.'/'.$e))
				{
	...
	{
		// Basename
		$basename = basename($filename, '.'.config_item('lack_data_extension'));
		
		$entry = array(
			'filename'    => $filename,
			'raw_content' => self::$ci->parser->parse(config_item('lack_data_path').'/'.self::$entry_dir.'/'.$filename, self::$data, true),
		...
		// Time
		$entry['time'] = filemtime(config_item('lack_data').'/'.self::$entry_dir.'/'.$filename);
	...
	public static function partial($view = null, $data = null, $return = false)
	{
		...
		// In 'core' subfolder
		if (file_exists(config_item('lack_templates').'/core/'.$view.'.php'))
		{
			$html = self::$ci->parser->parse(config_item('lack_template_path').'/core/'.$view, $data, true);
		}
		
		// In 'partial' subfolder
		elseif (file_exists(config_item('lack_templates').'/partials/'.$view.'.php'))
		{
			$html = self::$ci->parser->parse(config_item('lack_template_path').'/partials/'.$view, $data, true);
		}
		
		// In root of templates
		elseif (file_exists(config_item('lack_templates').'/'.$view.'.php'))
		{
			$html = self::$ci->parser->parse(config_item('lack_template_path').'/'.$view, $data, true);
		}
	...
	protected static function _setup()
	{
		...
		self::$ci->load->library('markdown', null, null);
